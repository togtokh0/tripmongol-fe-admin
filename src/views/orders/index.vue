  ...
  >
    <div class="p-8 text-sm text-gray-800">
      <div class="flex justify-between">
        ...
        <div>
          <Button className="" type="button" title="Нэмэх" :loading="false" />
        </div>
      </div>
...
export default defineComponent({
  ...
  computed: {
    filterData() {
      ...
      return data.filter(({ order_id }) => ("-" + order_id).includes(keyword));
    },
  },
  methods: {
  ...
  mounted: async function () {
    ...
    const result = await axios.get("admin/orders/pay");
